...
WORM_OUTER_DIA = 9.8 + DELTA;
WORM_LENGTH = 20;
WORM_MESH = 1;
WORM_SHAFT_LENGTH = 50;

CABLE_MOUNT_OUTER_DIA = 16;
...
MOUNTING_HOLE_FACE_DIA = MOUNTING_HOLE_HEAD_DIA - DELTA;
...
module extruder_drive_assembly() {
  ...
  union() {
    ...
    translate([-(DD_FILAMENT_OFFSET + COG_TEETH_LENGTH/4), -((COG_TEETH_DIA + WORM_OUTER_DIA)/2 - WORM_MESH), -WORM_LENGTH/2 - BEARING_WIDTH])
    worm_drive_train();
  }
}
...
module positive() {
  union() {
    translate([-(DD_FILAMENT_OFFSET + COG_TEETH_LENGTH/4), -((COG_TEETH_DIA + WORM_OUTER_DIA)/2 - WORM_MESH) - DD_OUTER_DIA/2, -WORM_LENGTH/2 - BEARING_WIDTH])
